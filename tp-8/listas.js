...
function calcularPromedio(listaDeNumeros) {
  const suma = listaDeNumeros.reduce((acc, num) => acc + num, 0);
...
function encontrarNumeroMayor(listaDeNumeros) {
  return listaDeNumeros.reduce((valorAnterior, valorActual) => {
...
function encontrarPalabraMasCorta(listaDePalabras) {
  listaDePalabras = listaDePalabras.sort();
...
function contarAprobados(listaDeNotas) {
  return listaDeNotas.filter((nota) => nota >= 7).length;
}
console.log(
...
function enumerarLista(listaDePalabras) {
  const listaEnumerada =
    listaDePalabras.slice(0, -1).join(", ") +
    " y " +
    listaDePalabras[listaDePalabras.length - 1];
  return listaEnumerada.charAt(0).toUpperCase() + listaEnumerada.slice(1) + ".";
}
